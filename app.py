...
class User(db.Model):
    ...
    role          = db.Column(db.String(20), default='seeker')  # seeker | company | admin
...
class Job(db.Model):
    ...
    job_type     = db.Column(db.String(30))  # دوام كامل | جزئي | عن بعد | عقد
    wilaya       = db.Column(db.String(50))
...
class Application(db.Model):
    ...
    status       = db.Column(db.String(20), default='new')  # new | reviewing | accepted | rejected
    created_at   = db.Column(db.DateTime, default=datetime.utcnow)
...
def apply(job_id):
    ...
    if 'cv' in request.files:
        ...
        if file and file.filename and allowed_file(file.filename):
            cv_filename = secure_filename(f"{session['user_id']}_{datetime.utcnow().timestamp()}_{file.filename}")
...
def register():
    if request.method == 'POST':
        ...
        role     = request.form['role']  # seeker | company
...
def dashboard():
    ...
    return render_template('dashboard.html', company=company,
        jobs=jobs_list, total_apps=total_apps, new_apps=new_apps,
        wilayas=WILAYAS, categories=CATEGORIES, job_types=JOB_TYPES)
...
def post_job():
    ...
    job = Job(
        ...
        salary_min   = int(request.form['salary_min'] or 0),
        salary_max   = int(request.form['salary_max'] or 0),
...
def company_applications():
    company = Company.query.filter_by(user_id=session['user_id']).first_or_404()
    apps = Application.query.join(Job).filter(Job.company_id == company.id)\
...
def update_app_status(app_id):
    company = Company.query.filter_by(user_id=session['user_id']).first_or_404()
    app_obj = Application.query.join(Job).filter(
        Application.id == app_id, Job.company_id == company.id).first_or_404()
...
def create_tables():
    with app.app_context():
        ...
        if not User.query.filter_by(email='admin@wazifati.dz').first():
            ...
            db.session.commit()
            print('✅ Admin account created: admin@wazifati.dz / admin2024')
