...
logger = logging.getLogger('wazifati')
...
try:
    ...
    app.secret_key = os.environ.get('SECRET_KEY', 'wazifati-fallback-2024')
...
def init_db():
    with app.app_context():
        ...
        if not User.query.filter_by(email='admin@wazifati.dz').first():
            admin = User(
                email='admin@wazifati.dz', role='admin',
                full_name='المشرف العام', is_active=True
            )
            admin.set_password('admin2024')
            db.session.add(admin)
            db.session.commit()
            logger.info("Admin account created: admin@wazifati.dz")
